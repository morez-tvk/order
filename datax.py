...
class Datax:
    def __init__(self, data, limit_time):
        ...
        # self.log = open(f"log/datax-{self.data['InstrumentCode']}--{time.time()}", "w")
        self.link = data['url']
        self.success = False
    ...
    def order(self):
        with FuturesSession(max_workers=500) as session:
            while True:
                future = session.post(url=self.link, headers=self.headers,cookies= self.cookies, json=self.data,
